...
All ClojureScript code you add to `src/javierrweiss/custom.cljs` is available
for use inside any [custom viewer code you
write](https://book.clerk.vision/#writing-viewers).

This is made possible by the code in `src/javierrweiss/sci_viewers.cljs`. If you
...
- [Create a GitHub repository](https://github.com/new). Ideally the owner
  matches `javierrweiss` and the project name is `clerkutiltest`.
